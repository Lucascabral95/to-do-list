...
import SinTareas from '../SinTareas/SinTareas';
import { useTableTasks } from '@/presentation/hooks';
import './Table.scss';

const Tablee: React.FC = () => {
  const { datos, selectedColor, filtroSeleccionado, setFiltroSeleccionado, isEmptyTask } =
    useTableTasks();
      ...
      {isEmptyTask ? (
        ...
        <>
          <div className="filtro-de-tareas">
            <Dropdown>
              <DropdownTrigger>
                <Button variant="bordered">Filtrar tareas</Button>
              </DropdownTrigger>
              <DropdownMenu aria-label="Filtrar tareas por estado">
                <DropdownItem onClick={() => setFiltroSeleccionado('Terminada')}>
                  Tareas completadas
                </DropdownItem>
                <DropdownItem onClick={() => setFiltroSeleccionado('Pendiente')}>
                  Tareas pendientes
                </DropdownItem>
                <DropdownItem onClick={() => setFiltroSeleccionado('sinRealizar')}>
                  Tareas vencidas
                </DropdownItem>
                <DropdownItem onClick={() => setFiltroSeleccionado('todas')}>
                  Todas
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
                ...
                {datos.length === 0 ? (
                  ...
                  datos.map((tabla, index) => (
                    <TableRow
                      ...
                      style={{
                        textDecoration:
                          moment(tabla.fechaDeTarea).isBefore(moment(), 'day') &&
